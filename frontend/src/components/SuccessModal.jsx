  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-[32px] p-8 max-w-sm w-full text-center shadow-2xl animate-in zoom-in duration-300">
        ...
        <h2 className="text-2xl font-black text-gray-800">{t('success_modal.title')}</h2>
        <p className="text-gray-500 mt-2">{t('success_modal.subtitle')}</p>

        <div className="my-6 p-6 bg-slate-50 rounded-3xl border-2 border-dashed border-gray-200">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('success_modal.id_label')}</p>
          ...
          {type === 'donor' && (
            <div className="mt-6 flex flex-col items-center">
              ...
              <p className="text-[10px] text-gray-400 mt-3 font-medium">{t('success_modal.scan_label')}</p>
            </div>
          )}
        </div>
